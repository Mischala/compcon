...
@Component({})
export default class GmPackBuilder extends Vue {
  private missionHeaders = [
    { text: 'Name', align: 'left', value: 'value.Name' },
  ]
  private encounterHeaders = [
    { text: 'Name', align: 'left', value: 'value.Name' },
  ...
  private npcs: NpcDatagridValue[] = []
  private missions: MissionDatagridValue[] = []
  private encounters: EncounterDatagridValue[] = []
  private selectedNpcs: NpcDatagridValue[] = []
  private selectedMissions: MissionDatagridValue[] = []
  private selectedEncounters: EncounterDatagridValue[] = []
  private packName = ""
  ...
  @Watch('selectedEncounters', { deep: true })
  private onSelectedEncountersChange (newVal, oldVal): void {
    // if lengh of new is bigger than old, we have a new missiom, and must lock it's children
    if (newVal.length > oldVal.length) {
      const addedvalues = newVal.filter(val => !oldVal.includes(val))
  ...
  private lockMissionChildren(mission: MissionDatagridValue): void {
  ...
  private unlockMissionChildren(mission: MissionDatagridValue): void {
    const encounters: EncounterDatagridValue[] = mission.value.Encounters.map(
      enc => _.find(this.encounters, (dataGridItem) => dataGridItem.value.ID === enc.ID )
    )
    encounters.map(enc => {
  ...
  private lockEncounterChildren(encounter: EncounterDatagridValue): void {
  ...
  private unlockEncounterChildren(encounter: EncounterDatagridValue): void {
    const npcs: NpcDatagridValue[] = encounter.value.Npcs("all").map(
      npc => _.find(this.npcs, (dataGridItem) => dataGridItem.value.ID === npc.ID )
    )
    npcs.map(enc => {
  ...
  private async exportGmPack(): Promise<void> {
    const missionStore = getModule(MissionStore, this.$store)
    const encounterStore = getModule(EncounterStore, this.$store)
    const npcStore = getModule(NpcStore, this.$store)
